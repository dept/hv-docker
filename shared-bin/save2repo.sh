...
if [ ! `git branch --list $branch_name` ]; then
   printTitle "- Branch name $branch_name doesn't exist yet - creating."
...
# git config user.name "HV Build"
# git config user.email "hvdevice@gmail.com"
# git config http.postBuffer 157286400

rsync -ac ../$source_directory/ . --delete --exclude .gitignore --exclude .git


git add -u
git add -A .
last_commit_messages="$(git diff --color=never --staged .gitlog | egrep "^\+[^\+]")"
